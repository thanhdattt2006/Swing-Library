...
public class LoanHistoryPanel extends JPanel {
				...
				for (Loan_Master m : currentList) {
					...
					if (m.getDue_date() != null) {
						try {
							...
							boolean shouldSend = (type == 3 && diff >= 2 && diff <= 3)
									|| (type == 1 && diff >= 0 && diff <= 1);
							...
							if (shouldSend) {
								...
								html = html.replace("{{name}}", m.getUsername())
										.replace("{{id}}", String.valueOf(m.getId())).replace("{{due_date}}",
												new SimpleDateFormat("dd-MM-yyyy").format(m.getDue_date()));

								if (mailModel.send("thanhdattt2006@gmail.com", m.getUsername(), subject, html))
									sent++;
								else
									fail++;
							} else {
			...
			protected void done() {
				try {
					...
					String resultMsg = String.format("Process completed!\n- Sent: %d\n- Failed: %d\n- Skipped: %d",
							res[0], res[1], res[2]);
					JOptionPane.showMessageDialog(LoanHistoryPanel.this, resultMsg, "Result",
							JOptionPane.INFORMATION_MESSAGE);
				} catch (Exception e) {
					loadingDialog.dispose();
					toggleButtons(true);
					JOptionPane.showMessageDialog(LoanHistoryPanel.this, "System error occurred!", "Error",
	...
	private void fillTable(List<Loan_Master> list) {
		...
		for (Loan_Master m : list) {
			tableModel.addRow(new Object[] { m.getId(), m.getUsername(), m.getEmployeeIdDisplay(),
					(m.getBorrow_date() != null ? sdf.format(m.getBorrow_date()) : ""), m.getStatus(),
					(m.getTotal_late_fee() + m.getTotal_compensation_fee()), "Details" });
		}
	}
	...
	private void openDetailsDialog(int loanId) {
		try {
			...
			dialog.setSize(1000, 550); // ✅ FIX KÍCH THƯỚC Ở ĐÂY
