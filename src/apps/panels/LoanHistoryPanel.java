...
public class LoanHistoryPanel extends JPanel {
    ...
    private void fillTable(List<Loan_Master> list) {
        ...
        for (Loan_Master m : list) {
            ...
            if (m.getStatus() != null) {
                statusDisplay = m.getStatus().toString(); 
            }
            
            String dateString = "";
            if (m.getBorrow_date() != null) {
                dateString = sdf.format(m.getBorrow_date());
            }

            double lateFee = m.getTotal_late_fee(); 
            double compFee = m.getTotal_compensation_fee(); 
            double totalFee = lateFee + compFee;

            tableModel.addRow(new Object[] { 
                m.getId(), 
                m.getUsername(), 
                m.getEmployeeIdDisplay(), 
                dateString,    
                statusDisplay, 
                totalFee, 
                "Details" 
